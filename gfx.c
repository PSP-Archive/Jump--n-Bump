...
//		flags |= SDL_FULLSCREEN;
	surfScreen = SDL_SetVideoMode (480, 272, 32, SDL_SWSURFACE);
	jnb_surface = SDL_CreateRGBSurface(SDL_SWSURFACE, 400, 256, 8, 0x000000ff, 0x0000ff00, 0x00ff0000, 0x00000000);
...
int get_color(int color, char pal[768])
{
	assert(color<256);
	assert(pal);
	SDL_MapRGB(surfScreen->format, (Uint8)(pal[color*3+0]<<2), (Uint8)(pal[color*3+1]<<2), (Uint8)(pal[color*3+2]<<2));
...
void flippage(int page)
{
	...
	SDL_BlitSurface(jnb_surface,NULL,surfScreen,&destRect);
	SDL_Flip(surfScreen);
}
